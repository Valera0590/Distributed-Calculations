...
namespace Load_Dist__Server_
{
    public partial class ServerForm : Form
    {
        ...
        private async void FindUnit (int diff, string idClient)
        {
            ...
            list.AddRange(root.TreeToList(root));
            ...
            if (indexUnit != -1)
            {
                root = root.FindIndex(root, indexUnit, diff);
                list.Clear();
                list.Add((root.ID, root.Level, root.ActuallyPower, root.MaxPower));
                list.AddRange(root.TreeToList(root));
                list.Capacity = list.Count;
                ...
                new Task(async () =>
                {
                    ...
                    root = root.DeleteLoad(root, indexUnit, diff);
                    list.Clear();
                    list.Add((root.ID, root.Level, root.ActuallyPower, root.MaxPower));
                    list.AddRange(root.TreeToList(root));
                    list.Capacity = list.Count;
            ...
            {
                if (indexUnit == -1) rtbProcessing.Text += "\n   -->   Процесс сложности " + diff + " не может выполниться ни на одном узле!\n";
                else rtbProcessing.Text += "\n   -->   Процесс сложности " + diff + " начал выполнение на узле "+indexUnit+"!\n";
            });
            PrintAsTree(list, indexUnit);
        ...
        private void PrintAsTree(List<(int, int, int, int)> list, int indexU)
        {
            for (int i = 0; i < list.Capacity; i++)
            {
                if (i == 0)
                    rtbProcessing.Invoke((MethodInvoker)delegate
                    {
                        rtbProcessing.Text += "  Главный узел " + list[i].Item1 + " : Загружен (" + list[i].Item3 + "/" + list[i].Item4 + ")\n";     // выводим полученное сообщение на форму
                    });
                else
                {
                    string lvl = "  ";
